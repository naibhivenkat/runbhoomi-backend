...
def _serialize_batsmen(db: Session, innings):
    ...
    batsmen = (
        ...
        .order_by(models.Batsman.is_striker.desc())
        .all()
    )
    ...
    for b in batsmen:
        ...
        fours = getattr(b, "fours", 0) or 0
        sixes = getattr(b, "sixes", 0) or 0

        strike_rate = round((runs * 100 / balls), 1) if balls > 0 else 0.0
        ...
        result.append({
            ...
            "is_striker": getattr(b, "is_striker", False),
        })
