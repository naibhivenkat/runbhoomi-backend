...
def create_groups(db: Session, tournament_id: str, teams: list, group_count: int) -> Optional[List[TournamentGroup]]:
    n = len(teams)
    ...
    for i in range(group_count):
        g = TournamentGroup(
            tournament_id=tournament_id, # Now gracefully handling the UUID string
    ...
    for i, group in enumerate(groups):
        ...
        for t in group_slice:
            db.add(GroupTeam(
                group_id=group.id,   # group.id is now a UUID string
                team_id=t.team_id    # t.team_id is now a UUID string
            ))
...
def round_robin(team_ids: List[str]) -> List[tuple[str, str]]:
    fixtures = []
...
def generate_knockout(team_ids: List[str]) -> List[tuple[Optional[str], Optional[str]]]:
    n = len(team_ids)
...
def paired_rounds(team_ids: List[str]) -> List[tuple[str, str]]:
    matches = []
...
def get_match_duration(overs: int) -> int:
    if overs <= 6:
        return 45
    elif overs <= 8:
        return 60
